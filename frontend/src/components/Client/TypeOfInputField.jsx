...
const TypeOfFieldInput = ({ data, ans, setAns, disabled, children }) => {
...
const MultipleChoice = ({ data, ans, setAns, disabled, button }) => {
  return (
    <div style={{ width: "100%" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
        ...
        <p style={{ marginBottom: 16 }}>{button}</p>
      </div>
      {data?.options?.map((item, index) => (
        <div key={index} style={{ marginBottom: 10 }}>
          <Radio
            color="#455ECE"
            value={ans}
            onChange={() => setAns(item)}
            checked={ans === item}
            disabled={ans !== item && disabled}
...
const Checkboxes = ({ data, ans, setAns, disabled, button }) => {
  //Multiple Choices
      ...
      {data?.options?.map((item, index) => (
        <div key={index} style={{ marginBottom: 10 }}>
          <Checkbox
            ...
            disabled={
              !multipleChoices.find((element) => element === item) && disabled
            }
            style={{ color: "#1F295A", fontSize: 16 }}
          >
            {item}
...
const DropDown = ({ ans, setAns, disabled, data, button }) => {
...
const Birthday = ({ data, ans, setAns, disabled, button }) => {
...
const Time = ({ data, ans, setAns, disabled, button }) => {
  return (
    <div style={{ width: "100%" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
        <p
