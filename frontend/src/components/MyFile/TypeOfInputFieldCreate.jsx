  ...
  if (data.questionType === "Short Answer")
    return (
      <DrossierInput
        ...
        onChange={(e) => setAns(data.requestQuestionId, e.target.value)}
        disabled={disabled}
      />
    );
      ...
      {data?.options?.map((item, index) => (
        <div key={index} style={{ marginBottom: 10 }}>
          <Radio
            color="#455ECE"
            value={ans}
            onChange={() => setAns(data.requestQuestionId, item)}
            checked={ans === item}
            disabled={disabled}
...
const Checkboxes = ({ data, ans, setAns, disabled }) => {
  ...
  const onChangeMultiChoices = (item) => {
    if (multipleChoices.find((element) => element === item)) {
      ...
      setAns(data.requestQuestionId, arr.join("/"));
    } else {
...
const DropDown = ({ ans, setAns, disabled, data }) => {
  ...
  useEffect(() => {
    if (ans?.length === 0) {
      setAns(data.requestQuestionId, data.options[0]);
    }
  }, [ans]);
          ...
          >
            {data?.options?.map((item, index) => (
              <div
                ...
                onClick={() => {
                  setAns(data.requestQuestionId, item);
                  setOpen(!open);
                }}
              >
...
const Birthday = ({ data, ans, setAns, disabled }) => {
  ...
  useEffect(() => {
    ...
    setAns(data.requestQuestionId, `${day}/${month}/${year}`);
  }, [day, month, year]);
